...
def _profile_in_session(connection_id: str, schema: str, table_name: str) -> dict:
    """Helper: run profile_table inside its own DB session (synchronous)."""
...
class DataProfilerAgent(BaseAgent):
    ...
    async def _execute_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If connection_id + schema + table_name are provided, runs a REAL profiling pass.
        Otherwise falls back to a simulated result (for demo mode).
        """
        ...
        if connection_id and table_name and schema:
            await self._log_activity(f"Profiling real table {schema}.{table_name} on connection {connection_id}")
            try:
                ...
                result = await loop.run_in_executor(
                    None,
                    lambda: _profile_in_session(connection_id, schema, table_name)
                )
                await self._log_activity(
                    f"Real profiling complete: {result['row_count']:,} rows, "
                    f"quality={result['quality_score']}%, issues={len(result.get('issues', []))}",
                    "success", {"quality_score": result["quality_score"]}
                )
                return result
            except Exception as exc:
                await self._log_activity(f"Real profiling failed: {exc}", "error")
                # Fall through to simulated result

        # ── Simulated profiling path (demo / no connection) ──────────────────
        table_id = task_data.get("table_id", "unknown")
        await self._log_activity(f"[Demo] Profiling table {table_id}")
        ...
        await self._log_activity(f"[Demo] Profiling completed for table {table_id}", "success", profile_results)
...
class AgentOrchestrator:
    ...
    def get_agent_status(self, agent_id: str = None) -> Dict[str, Any]:
        ...
        return {
            agent_id: agent.get_status().__dict__
